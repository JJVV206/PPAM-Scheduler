...
import { CalendarCheck2, Plus, Save, Trash2 } from "lucide-react";
      ...
      <div className="surface-panel flex flex-col gap-3 p-4 sm:flex-row sm:items-center sm:justify-between">
        <div className="min-w-0">
          <p className="font-heading text-xl font-semibold">
            {selected.size} horario{selected.size === 1 ? "" : "s"} marcado
            {selected.size === 1 ? "" : "s"}
          </p>
          <p className="text-sm text-muted-foreground">
            Marca los horarios en los que normalmente puedes servir. Las fechas
            especiales se agregan como excepciones.
          </p>
        </div>
        <div className="flex items-center gap-2 rounded-lg border border-border/70 bg-background/35 px-3 py-2 text-sm text-muted-foreground">
        ...
        <Card className="surface-panel">
          <CardContent className="space-y-5 p-6">
            {DAYS_OF_WEEK.map((day) => (
              <div key={day} className="rounded-lg bg-white/[0.03] p-4">
                ...
                <div className="grid gap-3 md:grid-cols-5">
                  {TIME_SLOTS.map((timeSlot) => {
                    ...
                    return (
                      <label
                        key={timeSlot}
                        className="flex items-center gap-3 rounded-lg border border-border/70 bg-background/30 px-4 py-3"
                      >
                        <Checkbox
          ...
          <Card className="surface-elevated h-fit">
            <CardContent className="space-y-5 p-6">
              <div className="space-y-2">
                ...
                <p className="text-sm text-muted-foreground">
                  Usa estos controles cuando no puedas recibir turnos por un
                  tiempo o por fechas específicas.
                </p>
              </div>
              <div className="flex items-center justify-between gap-4 rounded-lg bg-background/40 p-4">
                <div>
                  <p className="font-medium">No disponible temporalmente</p>
                  <p className="text-sm text-muted-foreground">
                    Pausar asignaciones y suplencias mientras esté activo
                  </p>
                </div>
                <Switch
          ...
          <Card className="surface-elevated">
            <CardContent className="space-y-4 p-6">
              <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                <div>
                  <p className="font-heading text-xl font-semibold">
                    Fechas en que no estaré disponible
                  </p>
                  <p className="text-sm text-muted-foreground">
                    Agrega vacaciones, viajes o cualquier periodo puntual.
                  </p>
                </div>
                <Button
                  type="button"
              ...
              {exceptions.length ? (
                ...
                <div className="rounded-lg border border-dashed border-border/70 bg-background/25 px-4 py-6 text-sm text-muted-foreground">
                  No hay fechas bloqueadas. Se tomará en cuenta tu
                  disponibilidad semanal.
                </div>
              )}

              <div className="bg-primary/8 rounded-lg px-4 py-3 text-sm text-muted-foreground">
                {exceptions.length &&
                exceptions[exceptions.length - 1]?.startDate
                  ...
                  : "Las fechas bloqueadas evitan que recibas turnos en esos días."}
              </div>
