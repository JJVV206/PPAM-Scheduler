...
export default async function VolunteerOpenSlotsPage() {
  ...
  if (!canAccessVolunteerRoute(dashboard.volunteer, "openSlots")) {
    return (
      <EmptyState
  ...
  return model.visibleOpenSlots.length ? (
    <div className="space-y-6">
      <section className="surface-panel p-4 sm:p-5">
        <h1 className="font-heading text-3xl font-semibold">
          {config.copy.openSlotsTitle}
        </h1>
        <p className="mt-2 max-w-2xl text-sm leading-6 text-muted-foreground">
          {config.copy.openSlotsDescription} Acepta solo si puedes cubrir el
          horario completo.
        </p>
      </section>
      <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">
        {model.visibleOpenSlots.map((slot) => (
          <OpenSlotCard
